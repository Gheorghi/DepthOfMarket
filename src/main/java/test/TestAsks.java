...
public class TestAsks {
    ...
    void checkAsksIsSortedAsDescending() {
        ...
        for (int i = 0; i<asksSorted.length; i++){
            assertEquals(Double.parseDouble(asksSorted[i][0]),asks.orders.get(i).getPrice());
            assertEquals(Double.parseDouble(asksSorted[i][1]),asks.orders.get(i).getVolume());
        }
    }

    ArrayList<Map<String, String>> setTestData(String[][] asks) {
        ArrayList<Map<String, String>> ordersList = new ArrayList<>();
        for (String[] order : asks) {
