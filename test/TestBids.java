...
public class TestBids {
    ...
    void checkBidssIsSortedAsDescending() {
        ...
        for (int i = 0; i<bidsSorted.length; i++){
            assertEquals(Double.parseDouble(bidsSorted[i][0]),bids.getOrders().get(i).getPrice());
            assertEquals(Double.parseDouble(bidsSorted[i][1]),bids.getOrders().get(i).getVolume());
        }
    }
